...
const MatchPanel = ({matchKey, onDisconnectMatch}) => {
    ...
    const onAddPoint = (indexTeam) => {
        const newMatchDetails = apiAddPoint(indexTeam, matchDetails)
        if (newMatchDetails === false) onDisconnectMatch()
        setMatchDetails(newMatchDetails)
    }

    const onEndMatch = (indexTeam) => {
        const newMatchDetails = endMatch(indexTeam, matchDetails)
        setMatchDetails(newMatchDetails)
    ...
    for (let i=0; i<= matchDetails["setNow"]; i++) {
        el_sets.push(
            <MatchPanelRow 
                ...
                special={i == matchDetails["setNow"] && matchDetails["winner"] == -1}
            />
        )
    }
    ...
    if (matchDetails["winner"] === -1) {
        ...
        el_btn = (
            <View style={styles.container_addPoint}>
                <Button mode="contained" onPress={() => onAddPoint(0)} style={styles.btn_addPoint}>+</Button>
                <Button mode="contained" onPress={() => onAddPoint(1)} style={styles.btn_addPoint}>+</Button>
            </View>
        )
    }
    else {
        el_btn = (
            <Text style={styles.txt_winner}>Drużyna {matchDetails["teams"][matchDetails["winner"]]["name"]} wygrała</Text>
        )
    }
...
async function loadMatchDetailsFromApi (matchKey, setMatchDetails, onDisconnectMatch) {
    const matchDetails = apiGetMatchDetails(matchKey)
